...
BEENGINE_API void _${COMPONENT_NAME}_registered(${COMPONENT_NAME} *comp, UICanvas *canvas) {
    if (comp->event_registered) comp->event_registered(comp, canvas);
    sprintf(comp->displayName, "${COMPONENT_NAME}_%d", comp->id);
}

BEENGINE_API void _${COMPONENT_NAME}_tick(${COMPONENT_NAME} *comp, UICanvas *canvas) {
    if (comp->event_tick) comp->event_tick(comp, canvas);
}

BEENGINE_API void _${COMPONENT_NAME}_draw(${COMPONENT_NAME} *comp, UICanvas *canvas) {
    if (comp->event_draw) comp->event_draw(comp, canvas);
}

BEENGINE_API void _${COMPONENT_NAME}_destroyed(${COMPONENT_NAME} *comp, UICanvas *canvas) {
    if (comp->event_destroyed) comp->event_destroyed(comp, canvas);
}

BEENGINE_API void _${COMPONENT_NAME}_clicked(${COMPONENT_NAME} *comp, UICanvas *canvas) {
    if (comp->event_clicked) comp->event_clicked(comp, canvas);
}

BEENGINE_API void _${COMPONENT_NAME}_pressed(${COMPONENT_NAME} *comp, UICanvas *canvas) {
    if (comp->event_pressed) comp->event_pressed(comp, canvas);
    engineCore_setFocusedUIComponent(canvas, comp);
}

BEENGINE_API void _${COMPONENT_NAME}_released(${COMPONENT_NAME} *comp, UICanvas *canvas) {
    if (comp->event_released) comp->event_released(comp, canvas);
}

BEENGINE_API void _${COMPONENT_NAME}_hovered(${COMPONENT_NAME} *comp, UICanvas *canvas) {
    if (comp->event_hovered) comp->event_hovered(comp, canvas);
}

BEENGINE_API void _${COMPONENT_NAME}_unhovered(${COMPONENT_NAME} *comp, UICanvas *canvas) {
    if (comp->event_unhovered) comp->event_unhovered(comp, canvas);
}

BEENGINE_API void _${COMPONENT_NAME}_input(${COMPONENT_NAME} *comp, UICanvas *canvas) {
